...
class NCT(DatasetBase):
    ...
    def __init__(self, root, preprocess):
        ...
        self.len = 0
        self.classes = ["Adipose", "Background", "Debris", "Lymphocytes", "Mucus", "Smooth muscle",
           "Normal colon mucosa", "Cancer-associated stroma", 
           "Colorectal adenocarcinoma epithelium"]
    ...
    def read_data(self, data_path, split):
        ...
        for label, folder in enumerate(folders):
            ...
            for imname in imnames:
                ...
                item =  Datum(impath=impath, label=label, classname=classname)  # {'impath':impath, 'label':label, 'classname':classname}
                items.append(item)
                data_count += 1
        print(data_count)   
        self.len = data_count     
