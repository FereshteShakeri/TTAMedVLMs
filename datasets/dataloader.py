...
from .utils import LoadImage, LoadImageRetina, ImageScaling, CopyDict
...
def get_dataloader_splits_radio(dataframe_path, data_root_path, targets, shots_test="60%", batch_size_test=128, num_workers=0, seed=0, size=(224, 224)):

    # Prepare data transforms for pre-processing
    transforms = Compose([LoadImage(size=size, canvas=True)])
    ...
    for i in range(len(dataframe)):
        ...
        # Image label
        data_i["label"] = int(np.argmax([sample_df[iTarget] for iTarget in targets]))
        data.append(data_i)
...
def get_dataloader_splits_retina(dataframe_path, data_root_path, targets_dict, transforms, shots_train="80%", shots_val="0%",
                          shots_test="20%", balance=False, batch_size=128, num_workers=0, seed=0, task="classification",
                          size=(224, 224), resize_canvas=False, batch_size_test=128):
    ...
    if task == "classification":
        ...
        transforms = Compose([CopyDict(), LoadImageRetina(), ImageScaling()])
    ...
    """
    transforms = Compose([CopyDict(), LoadImageRetina(), ImageScaling(size=size)])
    # Load data from dict file in txt
    data = []
    dataframe = pd.read_csv(dataframe_path)
    for i in range(len(dataframe)):
