...
def get_lora_parameters(model, bias='none'):
    ...
    for name, param in model.named_parameters():
        if bias == 'none':
            ...
            if 'lora_' in name or 'bias' in name or 'LayerNorm' in name:
                params.append(param)
        elif bias == 'ln':
...
def apply_lora_for_medclip(args, medclip):
    list_lora_layers = []
    if args.encoder == 'text' or args.encoder == 'both':
        indices = INDEX_POSITIONS_TEXT[args.position]
        text_encoder = medclip.text_model.model.encoder
        for i, block in enumerate(text_encoder.layer):
            #print(f"Bert Layer {i}: {block}")
            if i in indices:
                for item in args.params:
                    if item == 'q':
                        
                        block.attention.self.query = LinearLoRA(block.attention.self.query,
                                                          r=args.r,
                                                          lora_alpha=args.alpha,
                                                          fan_in_fan_out=False,
                                                          dropout_rate = args.dropout_rate)
                    elif item == 'k':
                        block.attention.self.key = LinearLoRA(block.attention.self.key,
                                                          r=args.r,
                                                          lora_alpha=args.alpha,
                                                          fan_in_fan_out=False,
                                                          dropout_rate = args.dropout_rate)
                    elif item == 'v':
                        block.attention.self.value = LinearLoRA(block.attention.self.value,
                                                          r=args.r,
                                                          lora_alpha=args.alpha,
                                                          fan_in_fan_out=False,
                                                          dropout_rate = args.dropout_rate)
                        
                list_lora_layers.append(block.attention.self)

    if args.encoder == 'vision' or args.encoder == 'both':
        indices = INDEX_POSITIONS_VISION[args.backbone][args.position]
        vision_encoder = medclip.vision_model.model.encoder
        index =0
        for i,block in enumerate(vision_encoder.layers):
            for j, children_block in enumerate(block.blocks):
                #print(f"SwinLayer {index}: {children_block}")
                index +=1
                for item in args.params:
                    if item == 'q':
                        
                        children_block.attention.self.query = LinearLoRA(children_block.attention.self.query,
                                                          r=args.r,
                                                          lora_alpha=args.alpha,
                                                          fan_in_fan_out=False,
                                                          dropout_rate = args.dropout_rate)
                    elif item == 'k':
                        children_block.attention.self.key = LinearLoRA(children_block.attention.self.key,
                                                          r=args.r,
                                                          lora_alpha=args.alpha,
                                                          fan_in_fan_out=False,
                                                          dropout_rate = args.dropout_rate)
                    elif item == 'v':
                        children_block.attention.self.value = LinearLoRA(children_block.attention.self.value,
                                                          r=args.r,
                                                          lora_alpha=args.alpha,
                                                          fan_in_fan_out=False,
                                                          dropout_rate = args.dropout_rate)
                        
                list_lora_layers.append(children_block.attention.self)
    return list_lora_layers


def apply_lora_for_retclip(args, retclip):
    list_lora_layers = []
    if args.encoder == 'text' or args.encoder == 'both':
        indices = INDEX_POSITIONS_TEXT[args.position]
        text_encoder = retclip.bert.encoder
        for i, block in enumerate(text_encoder.layer):
            #print(f"Bert Layer {i}: {block}")
            if i in indices:
                for item in args.params:
    ...
    if args.encoder == 'vision' or args.encoder == 'both':
        indices = INDEX_POSITIONS_VISION[args.backbone][args.position]
        vision_encoder = retclip.visual.transformer
        for i, block in enumerate(vision_encoder.resblocks):
            #print(f"Residual Attention Block {i}: {block}")
            if i in indices:
                for name, submodule in block.named_children():
